...
begin   (*  PRINCIPAL  *)
   ...
   While (p <= Tope) and estado do
     Begin
            ...
            while not (c in ['+','-','*','/','(',')']) and ((p<= Tope) and st) do
             Begin
               Case c of
                   ...
                   '0'..'9' : Begin
                               while (p<= Tope) and (c in ['0'..'9','.']) do
                                ...
                                if ChkNumero(texto) and
                                    (SigChar(expresion,p,PtrPos) in ['A','C','E','I','L','S','T','X','a']) then
                                     Begin
                   ...
                   ' ' : if (SigChar(expresion,p+1,PtrPos) in ['0'..'9','X','a']) and
             ...
             Else
               if (c in ['*','/','+','-','^','u',')']) and Es_FunLIB(texto) then
                  Begin
                    Estado:= false;
                    Error:= 11;
                    ErrorTexto:= texto;
       ...
       if (P <= Tope) and (expresion[p] in ['*','/','+','-','^','u']) and Estado then
  ...
  if Error >0 then
    Begin
      Case Error Of
       1 : writeln('Error: Falta Base para la potencia ('+ErrorTexto+')');
       2 : writeln('Error: Falta ¡ndice para la radicaci¢n ('+ErrorTexto+')');
       3 : writeln('Error: Trmino nulo. Error en signo de ('+ErrorTexto+')');
       4 : writeln('Error: Factor nulo. Error de signo en ('+ErrorTexto+')');
       5 : writeln('Error: Identificador Desconocido: '+ErrorTexto);
       6 : writeln('Error: Falta un "("');
       7 : writeln('Error: Falta un ")"');
       8 : writeln('Error: Falta un Operador. '+ErrorTexto);
       9 : writeln('Error: Falta el Radicando en la Operaci¢n û');
      10 : writeln('Error: Falta exponente en la Operaci¢n ^');
      11 : writeln('Error: Funcion '+ErrorTexto+' sin argumento  ---> Por ej: '+ErrorTexto+'(x)');
      12 : writeln('Error: Expresi¢n nula entre parntesis');
      end;
    end;
  pos:=p;
  validar:=estado;
...
Begin
   ...
   if not validar(strg,p) then
      writeln(' La Formula ingresada es Incorrecta !!!')
   else Fnc := strg;
End;
